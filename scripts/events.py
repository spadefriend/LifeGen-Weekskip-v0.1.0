...
class Events:
    ...
    def one_moon(self):
        ...
        if Cat.dead_cats:
            ...
            if insert:
                if len(Cat.dead_cats) > 1 and game.clan.game_mode != 'classic':
                    ...
                    event = f"The past week, {insert} has taken their place in StarClan. {game.clan.name}Clan mourns their " \
        ...
        if not game.clan.your_cat.dead and game.clan.your_cat.status != 'exiled' and not game.clan.your_cat.outside:
            ...
            elif game.clan.your_cat.moons == 27 and game.clan.your_cat.shunned == 0:
            ...
            if game.clan.your_cat.moons >= 53:
    ...
    def check_achievements(self):
        ...
        for cat in clan_cats:
            ...
            if Cat.all_cats.get(cat).status == 'apprentice' and Cat.all_cats.get(cat).name.prefix == "Pea" and Cat.all_cats.get(cat).pelt.white_colours:
                achievements.add("33")
            if Cat.all_cats.get(cat).status == 'kitten' and Cat.all_cats.get(cat).moons > 27:
                achievements.add("34")
        ...
        if you.experience >= 400:
            achievements.add('26')
        if you.experience >= 800:
            achievements.add('27')
            ...
            try:
                ...
                if birth_type == BirthType.NO_PARENTS:
                    ...
                    parent2 = create_new_cat(Cat, Relationship,
                                                ...
                                                alive=False,
                                                thought=thought,
                                                age=parent1.moons + random.randint(1,21),
                                                outside=True)[0]
                    if not game.clan.clan_settings["same sex birth"]:
    ...
    def generate_events(self):
        ...
        if game.clan.your_cat.status == 'elder' and game.clan.your_cat.moons < 434:
            status = "young elder"
    ...
    def get_moon_freshkill(self):
        ...
        game.freshkill_event_list.append(f"The clan managed to catch {prey_amount} pieces of prey in this week.")
    ...
    def herb_gather(self):
        ...
        if game.clan.game_mode == 'classic':
            ...
            for med in meds_available:
                ...
                if not herbs_found:
                    event_list.append(
                        f"{med.name} could not find any herbs this week.")
                else:
                    try:
                        ...
                        event_list.append(
                            f"{med.name} gathered {insert} this week.")
                    except IndexError:
                        event_list.append(
                            f"{med.name} could not find any herbs this week.")
                        return
    ...
    def handle_lost_cats_return(self):
        ...
        for x in [lost_cat] + [Cat.fetch_cat(i) for i in additional_cats]:             
            ...
            if x.status in ["apprentice", "medicine cat apprentice", "mediator apprentice", "queen's apprentice", "kitten", "newborn"]:
                ...
                elif x.status in ["kitten", "newborn"] and x.moons >= 27:
    ...
    def perform_ceremonies(self, cat):
        ...
        if not cat_dead:
            ...
            if not cat.no_retire and cat.status in ['warrior', 'deputy'] and len(cat.apprentice) < 1 and cat.moons > 495:
                # There is some variation in the age. 
                if cat.moons > 608 or not int(random.random() * (-0.7 * cat.moons + 100)):
