...
class Welcoming_Events():
    ...
    def welcome_cat(clan_cat: Cat, new_cat: Cat) -> None:
        ...
        if new_cat.ID in clan_cat.relationships:
            if clan_cat.age == 1:
                clan_cat.relationships[new_cat.ID].log.append(interaction_str + f" - {clan_cat.name} was {clan_cat.moons} week old")
            else:
                clan_cat.relationships[new_cat.ID].log.append(interaction_str + f" - {clan_cat.name} was {clan_cat.moons} weeks old")

            new_cat.relationships[clan_cat.ID].link_relationship()

        if clan_cat.ID in new_cat.relationships:
            if new_cat.age == 1:
                new_cat.relationships[clan_cat.ID].log.append(interaction_str + f" - {new_cat.name} was {new_cat.moons} week old")
            else:
                new_cat.relationships[clan_cat.ID].log.append(interaction_str + f" - {new_cat.name} was {new_cat.moons} weeks old")
