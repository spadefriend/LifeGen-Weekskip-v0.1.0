...
class ProfileScreen(Screens):
    ...
    def build_profile(self):
        ...
        if self.the_cat.ID != game.clan.your_cat.ID and not self.the_cat.dead and not self.the_cat.outside and not game.clan.your_cat.dead and not game.clan.your_cat.outside and not game.clan.your_cat.moons < 0:
            ...
            cat_dead_condition_sc = self.the_cat.dead and not self.the_cat.df and (game.clan.your_cat.dead or (game.clan.your_cat.skills.meets_skill_requirement(SkillPath.STAR) and game.clan.your_cat.moons >=27) or self.the_cat.joined_df)
            cat_dead_condition_df = self.the_cat.dead and self.the_cat.df and (game.clan.your_cat.dead or (game.clan.your_cat.skills.meets_skill_requirement(SkillPath.DARK) and game.clan.your_cat.moons >=27) or game.clan.your_cat.joined_df)

            cat_dead_conditions = cat_dead_condition_sc or cat_dead_condition_df

            cat_alive_condition_sc = game.clan.your_cat.dead and not game.clan.your_cat.df and (self.the_cat.dead or (self.the_cat.skills.meets_skill_requirement(SkillPath.STAR) and self.the_cat.moons >= 27))
            cat_alive_condition_df = game.clan.your_cat.dead and game.clan.your_cat.df and (self.the_cat.dead or (self.the_cat.skills.meets_skill_requirement(SkillPath.DARK) and self.the_cat.moons >= 27))
        ...
        if self.the_cat.status == 'leader' and not self.the_cat.dead:
            ...
            self.profile_elements["halfmoon"] = UIImageButton(scale(pygame.Rect(
                ...
                tool_tip_text= "You may attend the half-moon gathering every four weeks",
                manager=MANAGER
            )
            if self.the_cat.dead or self.the_cat.outside or (game.clan.age % 4 != 0):
        ...
        if previous_open_tab == 'history' and sub_tab_switch is False:
            ...
            if self.open_sub_tab != 'life events':
                ...
                self.no_moons = UIImageButton(scale(pygame.Rect(
                    ...
                    tool_tip_text='Show the Week that certain history events occurred on', manager=MANAGER
                )
                self.show_moons = UIImageButton(scale(pygame.Rect(
                    (104, 1028), (68, 68))),
                    "",
                    object_id="#checked_checkbox",
                    tool_tip_text='Stop showing the Week that certain history events occurred on', manager=MANAGER
                )
        ...
        if beginning:
            if self.the_cat.df == False:
                if beginning['clan_born']:
                    text += " {PRONOUN/m_c/subject/CAP} {VERB/m_c/were/was} born on Week " + str(
                        beginning['moon']) + " during " + str(beginning['birth_season']) + "."
                else:
                    text += " {PRONOUN/m_c/subject/CAP} joined the Clan on Week " + str(
                        beginning['moon']) + " at the age of " + str(beginning['age']) + " Weeks."
            else:
                text += "<br>You met {PRONOUN/m_c/object} on Week " + str(beginning['moon']) + "."
        ...
        if app_ceremony:
            ...
            if int(grad_age) < 50:
                graduation_history += " {PRONOUN/m_c/poss/CAP} training went so well that {PRONOUN/m_c/subject} graduated early at " + str(
                    grad_age) + " weeks old."
            elif int(grad_age) > 57:
                graduation_history += " {PRONOUN/m_c/subject/CAP} graduated late at " + str(grad_age) + " weeks old."
            else:
                graduation_history += " {PRONOUN/m_c/subject/CAP} graduated at " + str(grad_age) + " weeks old."

            if game.switches['show_history_moons']:
                graduation_history += f" (Week {app_ceremony['moon']})"
        cat_dict = {
            "m_c": (str(self.the_cat.name), choice(self.the_cat.pronouns))
        }
            ...
            if victim_names:
                for name in victim_names:
                    if not moons:
                        ...
                        name_list.append(name + f" (Week {str(victim_names[name])})")
    ...
    def get_condition_details(self, name):
        ...
        if name in self.the_cat.permanent_condition:
            ...
            if self.the_cat.permanent_condition[name]["born_with"] is True:
                ...
                if moons_with != 1:
                    text_list.append(f"has had this condition for {moons_with} weeks")
                else:
                    text_list.append(f"has had this condition for 1 week")
        ...
        if name in self.the_cat.injuries:
            ...
            if moons_with != 1:
                text_list.append(f"{insert} {moons_with} weeks")
            else:
                text_list.append(f"{insert} 1 week")
    ...
    def toggle_dangerous_tab(self):
        ...
        if previous_open_tab == 'dangerous':
            ...
            if game.clan.your_cat.moons < 27:
                self.join_df_button.disable()
        ...
        # Roles Tab
            ...
            if self.the_cat.age in ['young adult', 'adult', 'senior adult', 'senior'] and not self.the_cat.dead and not self.the_cat.outside and game.switches['have kits']:
                self.have_kits_button = UIImageButton(scale(pygame.Rect((804, 1172), (344, 72))), "",
                                                    starting_height=2, object_id="#have_kits_button", tool_tip_text='You will be more likely to have kits the next week.',
                                                    manager=MANAGER)
            else:
                self.have_kits_button = UIImageButton(scale(pygame.Rect((804, 1172), (344, 72))), "",
