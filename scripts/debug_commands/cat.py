...
class listCatsCommand(Command):
    ...
    def callback(self, args: List[str]):
        for cat in Cat.all_cats_list:
            add_output_line_to_log(f"{cat.ID} - {cat.name}, {cat.status}, {cat.moons} weeks old")
...
class ageCatsCommand(Command):
    ...
    def callback(self, args: List[str]):
        ...
        for cat in Cat.all_cats_list:
            if str(cat.name).lower() == args[0].lower() or cat.ID == args[0]:
                if len(args) == 1:
                    add_output_line_to_log(f"{cat.name} is {cat.moons} weeks old")
                    return
                else:
                    ...
                    add_output_line_to_log(f"{cat.name} is now {cat.moons} weeks old")
