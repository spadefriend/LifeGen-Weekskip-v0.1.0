    ...
    def handle_injuries(cat, other_cat=None, alive_kits=None, war=None, enemy_clan=None, season=None):
        ...
        if text is not None:
            ...
            if other_cat:
                ...
                if cat.moons == 1:
                    cat.relationships[other_cat.ID].log.append(log_text + f" - {cat.name} was {cat.moons} week old")
                else:
                    cat.relationships[other_cat.ID].log.append(log_text + f" - {cat.name} was {cat.moons} weeks old")
                ...
                if not int(random.random() * chance):
                    ...
                    if cat.age == 'adolescent':
                        ...
                        if not game.clan.leader.dead and not game.clan.leader.exiled and \
                                not game.clan.leader.outside and cat.moons < 521:
